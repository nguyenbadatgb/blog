...
function getTags()
{
    ...
    $tagsId = array_column($Tags, '0');
    $tagsId = implode(',', $tagsId);

    $QueryPosts  = "SELECT * FROM blog_post WHERE Post_Tag in (".$tagsId.")";
    $ResultPosts = $Connection->query($QueryPosts);
    $posts = $ResultPosts->fetch_all();
    $tagsImage = [];
    $postsTotal = [];
    foreach($posts as $post) {
        $tagsImage[$post['2']] = $post['3'];
        $postsFactory[$post['2']] = ($postsFactory[$post['2']] ?? 0) + 1;
    ...
    if ($Result->num_rows > 0) {
        foreach ($Tags as $Row) {
            $TagName  = $Row['1'];
            $TagId  = $Row['0'];
            echo '<div class="swiper-slide" data-swiper-slide-index="1" role="group" aria-label="1 / 15" style="width: 159.8px; margin-right: 20px;">
													<div class="card-style-1"><a href="#">
															<div class="card-image"><img src="'.($tagsImage[$TagId] ?? 'images/IMG_4935.JPG').'" style="width: 160px; height: 195px;" alt="Genz">
																<div class="card-info">
																	<div class="info-bottom">
																		<h6 class="color-white mb-5">'.$TagName.'</h6>
																		<p class="text-xs color-gray-500">'.($postsTotal[$TagId] ?? '0').' Articles</p>
																	</div>
																</div>
															</div>
														</a></div>
